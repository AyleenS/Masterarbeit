...
\begin{Prop}[Kommunikationsfehler-Traces und Implementierung]
  ...
  \begin{enumerate}
    ...
    \item \emph{Input-kritische-Traces}: $\MIT (P) = \left\{wa\in\Sigma ^* \mid
      ...
      \right.$\linebreak $\left. a\in I\land p'\nmust[a]_{P'} \right\}$ \TODO{erzwungen Zeilenumbruch kontrollieren}
  \end{enumerate}
\end{Prop}
\begin{proof}\mbox{}
  \begin{enumerate}
...
\begin{proof}\mbox{}\\
  ...
  \begin{itemize}
    \item Fall 1 ($w\in\MIT _{12}$): Aus der Definition von \MIT{} folgt, dass
      ...
      \TODO{ab hier Transitionen ihren Automaten zuordne}
    \begin{itemize}
    ...
    \item Fall II ($v_1\in\PrET _1$): $\exists u_1\in O_1^*: p_{01}
      ...
      \begin{itemize}
        \item Fall IIa) \big($u_2\in (O_1\cap I_2)^*,c\in (O_1\cap I_2)$, sodass
          ...
          $c$ Output von $P_1$ zu koppeln wäre. Es handelt sich also um einen
        ...
        \item Fall IIb) \big($p_1\weakmust[u_2]p'_2$ mit $u_1=u_1|_{I_2}$\big): Es gilt
          ...
          geerbten Fehler. Nun wird $v:=\prune (v'u_1)\in\PrET _{12}$ gewählt,
    ...
    \item Fall 2 ($w=x_1\dots x_n x_{n+1}\in\Sigma ^+$ mit $n\geq 0$ und
      ...
      \begin{itemize}
        \item Fall 2a) ($w\in\MIT _1$): In $U\|P_1$ erhält man $(p_1,p_{01})
          ...
          Aktionen aus $w$ bis auf $x_{n+1}$ synchronisiert werden und $I\cap
          I_U=\emptyset$, gilt $x_1,\dots x_n\in O_{U\|P_1}$. Da $(p_1,p')\in
          E_{U\|P_1}$ gibt es mindestens eine as"=Implemnentierung $P'$ von
          $U\|P_1$, die diesen Fehler-Zustand ebenfalls enthält. Daraus ergibt
          sich dann $\varepsilon\in\PrET (P')$.
          \TODO{ab hier Definitionen mit as-Implementierungen beachten}
        \item Fall 2b) ($w\in\PrET _1$): In $U\|P_1$ erhält man $(p_0,p_{01})
          ...
          $wu\in\StET (U\|P_1)$. Da alle Aktionen in $w$ synchronisiert werden
      ...
      \begin{itemize}
        \item Fall 2i) (neuer Fehler): Da jeder Zustand von $U$ alle Inputs
          ...
          Präfix von $w$ in $\ET _2$ enthalten.
...
\begin{proof}\mbox{}\\
  \TODO{Beweis auf die Definitionen anpassen (as-Implementierungen!!!)}
