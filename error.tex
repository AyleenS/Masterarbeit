...
% Um
% die Sprache eines \MEIO{}s definieren zu können, wurden die strikten Traces
% bereits in Definition~\ref{strikteKommTracesDef} ausgeführt. Da es jedoch in
% diesem Kapitel speziell um die Kommunikationsfehler geht, werden sie in der
% nachstehenden Definition noch einmal mit aufgeführt.
...
Traces alle seiner as"=Implementierungen.

\begin{Prop}[Kommunikationsfehler-Traces und Implementierung]
  \label{KommTracesProp}
  Für ein \MEIO{} $P$ gilt:

  \TODO{überlegen, was von dieser Proposition noch gilt:}\\
  \TODO{\ET{} wahrscheinlich nur noch für $\cont (\PrET (P))$,}\\
  \TODO{\MIT{} nur noch auf fehler-freien Traces}

  \begin{enumerate}
    \item strikte Kommunikationsfehler-Traces: $\StET (P) =
...
\begin{proof}\mbox{}\\
  ...
  Kommunikationsfehler-Traces in $\MIT _{12}$ oder in $\PrET _{12}$ enthalten.
  \begin{itemize}
...
\TODO{ab hier weiter durchlesen}
