...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item $\begin{aligned}[t] \must _{P'}=\may _{P'} &=
        ...
        &\cup \left\{(q'_j,\alpha ,q_k)\mid p_j\may[\alpha] p_k,
        p_j\nmust[\alpha] p_k\right\}\\
        &\cup \left\{(q'_j,\alpha ,q'_k)\mid p_j\may[\alpha] p_k,
        p_j\nmust[\alpha] p_k\right\},
    \end{aligned}$
    \item $E_{P'}=\emptyset$.
  \end{itemize}
  ...
  und gestrichenen Zustand implementiert werden. Die Zustände $q$ ohne Strich
  von $P'$ sind bis auf $q_0$ die potentiell ruhigen Zustände, die nur die
  zwingend notwendigen Transitionen besitzen. Die gestrichenen Zustände $q'$
  aus $P'$ hingegen sind die Zustände, die das gesamte erlaubte Verhalten
  umsetzten. Durch die Transitionen, die von den gestrichenen Zuständen zu den
  ungestrichenen führen, sind alles erlaubte möglich bis entschieden wird, nur
  noch das geforderte zu benötigen. Da die Menge der Fehler-Zustände leer ist,
  gilt~\ref{SimDef}.1 für $\mathcal{R}$. Die must"=Transitionen werden für die
  ungestrichenen und gestrichenen Zustände umgesetzt, dies erfüllt zusammen mit
  $\mathcal{R}$ die Definition~\ref{SimDef}.2. Ebenso wird der dritte Punkt der
...
\begin{proof}
  ...
  \begin{itemize}
    \item Fall a) ($\varepsilon\in\ET (P'_2\|T)$): Es handelt sich bei dem
      ...
      neuer Fehler in der Parallelkomposition entstanden sein. Es gilt, dass in
      $P'_2$ ein Präfix von $w$ in $\ET _{P'_2}$ enthalten ist, wegen des
      ...
      \cont{} abgeschlossen, somit gilt $w\in\ET _{P'_1}\subseteq\QET _{P'_2}$.
