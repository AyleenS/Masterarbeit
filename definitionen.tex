...
\begin{Def}[Parallelkomposition]
  ...
  $P_1$ ist ein \emph{Partner} von $P_2$, wenn die Parallelkomposition von
...
und keine Fehler-Zustände enthalten, entspricht die as"=Verfeinerung einer Bisimulation. Dafür müssen die
...
Die schwache Simulation erlaubt interne Aktionen des \MEIO{}, das die
entsprechende Aktion matchen muss. Jedoch ist es zwingend notwendig, dass ein
Input sofort ausgeführt wird und erst dann interne Aktinen möglich sind, da
...
somit nicht lokal kontrolliert sind.\\
...
Mengen kann z.B.\ aus~\cite{Vogler2014EIO} übernommen werden. Die
Aktionsfolgen eines \MEIO{} werden auch \emph{Traces} genannt. Traces sind die
möglichen Wege des Systems, mit ihrer Transitionsbeschriftung. Diese
Beschriftung besteht aus Inputs und Outputs, mit denen die Transitionsfolge ab
dem Startzustand beschriftet ist. Somit kann ein Trace auch als das Wort
aufgefasst werden, dass verarbeitet wird während des Ablauf des Systems.
