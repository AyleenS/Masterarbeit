...
% \begin{Def}[strikte Kommunikationsfehler-Traces]
%   \label{strikteKommTracesDef}
%   Für ein \MEIO{} $P$ sind die \emph{strikte Kommunikationsfehler-Traces} wie
%   folgt definierte Wort-Menge: $\StET (P) := \left\{w\in\Sigma ^* \mid p_0 \weakmay[w]_P p
%   \in E\right\}$.
% \end{Def}
...
\begin{Def}[Sprache]
  ...
  %erreichen: 
  ist $L(P) := \left\{w\in \Sigma ^* \mid p_0\weakmay[w]_P %p\notin E
  \right\}$.
  %\cup \cont (\StET (P))$.
\end{Def}
