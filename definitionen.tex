...
Input sicherstellt, der synchronisiert wird, wird dieser Input in der
Parallelkomposition nicht mehr garantiert. Es kann also in der Kommunikation
mit einem weiteren \MEIO{} an dieser Stelle zu einem neuen Fehler kommen.
...
komponiert werden. Das Wort $w_1\|w_2$ ist dann eine Aktionsfolge aus Inputs
und Outputs. Es existiert in einer Parallelkomposition eine Transitionsfolge
vom Startzustand aus, die mit den Aktionen aus $w\in w_1\|w_2$ beschriftet ist.
Ein Trace aus der Menge $w_1\|w_2$ der Parallelkomposition kann also auch als
Wort aufgefasst werden, das verarbeitet wird während des Ablaufs des Systems.
