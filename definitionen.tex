...
\begin{Def}[Hiding-Operator]
  ...
  $P/L$, mit dem Hiding-Operator $\cdot /\cdot$, definiert als der \MEIO{}
  $P/L$ mit:
  \begin{itemize}
    \item $\tau \notin L$,
    \item $L\cap I = \emptyset$,
    \item $P/L = P$, $I_{P/L} = I_P$, $p_{0_{P/L}} = p_{0_P}$ und $E_{P/L} =
      E_P$,
    \item $\must _{P/L} = (\must _P \cup \{(p,\tau,p')\mid (p,o,p')\in \must
      _P, o \in L\})\backslash \{(p,o,p')\mid (p,o,p')\in \must _P, o \in L\}$,
    \item $\may _{P/L} = (\may _P \cup \{(p,\tau,p')\mid (p,o,p')\in \may _P, o
      \in L\})\backslash \{(p,o,p')\mid (p,o,p')\in \may _P, o \in L\}$.
  \end{itemize}
\end{Def}
