...
\begin{proof}\mbox{}\\
  ...
  Teilmenge der Sprache $L _P$. Es gibt also einen Trace für $w$ in $P$ der
  Form, wie sie in Lemma~\ref{AblaefeSchwVerfSpezLem} vorausgesetzt wird. Falls
  ein $q_k$ aus~\ref{AblaefeVerfSpezLem} für $0 \leq k \leq n$ in $E_Q$
...
zulässt. Dabei werden die lokalen Aktionen, die zu einem Fehler führen
abgeschritten. Außer dem universalen Zustand gibt es in einem \MIA{} wie
...
das beliebiges Verhalten im allgemeinen nicht geben.\\
...
der disjunktiven must"=Transitionen, haben die must"=Transitionen auch nur einen
Zustand als Ziel und keine ganze Menge. Im folgenden sollen \MIA{}s Modale
Transitionssysteme sein, die diese Voraussetzungen erfüllen. Dann zu jedem
\MEIO{} einen äquivalenten \MIA{}.\\
...
normalisierte \MEIO{}s entspricht sie also der Relation \wasRel{}.
...
\begin{Def}[Normalform]
  ...
  Input"=Aktionen $I$ sein. Der Zustand $e$ soll keine ausgehenden Transitionen
  besitzen.
  ...
  \begin{enumerate}[(i)]
    \item Definiere $\overline{E_P} = \{p \mid \exists p'\in E_P, w\in O^*:
      p\weakmay[w]_P p'\}$ und ersetzte $E_P$ durch $\overline{E_P}$. Der
      dadurch entstehenden \MEIO{} heißt $\overline{P}$.
...
Schritt (i) der Normalform Konstruktion verändert nicht an den Transitionen
...
\ERel{}. Es gilt also die folgende Proposition.
