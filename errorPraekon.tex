...
folgt dann mit Satz~\ref{ZusammenhWasAsSatz}.

\begin{Prop}[Abläufe in schwachen as"=Verfeinerungen und Spezifikationen]
  \label{AblaefeSchwVerfSpezProp}
  Sei $P'$ eine schwache as"=Verfeinerung eines \MEIO{}s $P$ und $\mathcal{R}$
  die schwache as"=Verfeinerungs"=Relation zwischen den beiden
  Transitionssystemen. Ein Ablauf $p'_0 \may[\alpha _1]_{P'} p'_1 \may[\alpha
  _2]_{P'} \dots p'_{n-1} \may[\alpha _n]_{P'} p'_n$ aus $P'$ kann durch einen
  Ablauf $p_0 \weakmay[\alpha _1]_P p_1 \weakmay[\alpha _2]_P \dots p_{n-1}
  \weakmay[\alpha _n]_P p_n$ mit $p'_j \mathcal{R} p_j$ für alle
  $j\in\{0,1,\dots n\}$ in $P$ gematched werden oder es wird ein $p_j$ für
  $0\leq j < n$ erreicht, dass in der Menge $E_P$ enthalten ist. Aus $w\in
  L(P')$ folgt also $w\in L(P)$ oder $w\in\cont (\StET (P))\subseteq \ET (P)$.
\end{Prop}
\begin{proof}
  \TODO{zu beweisen und nächste Proposition daraus folgen lassen}
\end{proof}

\begin{Prop}[Abläufe in as-Verfeinerungen und Spezifikationen]
  \label{AblaefeVerfSpezProp}
  Sei $P'$ eine as"=Verfeinerung eines \MEIO{}s $P$ und $\mathcal{R}$ die
  as"=Verfeinerungs"=Relation zwischen den beiden Transitionssystemen. Ein
  Ablauf $p'_0 \may[\alpha _1]_{P'} p'_1 \may[\alpha _2]_{P'} \dots p'_{n-1}
  \may[\alpha _n]_{P'} p'_n$ aus $P'$ kann durch einen Ablauf $p_0 \may[\alpha
  _1]_P p_1 \may[\alpha _2]_P \dots p_{n-1} \may[\alpha _n]_P p_n$ mit $p'_j
  \mathcal{R} p_j$ für alle $j\in\{0,1,\dots n\}$ in $P$ gematched werden oder
  es wird ein $p_j$ für $0\leq j < n$ erreicht, dass in der Menge $E_P$
  enthalten ist. Aus $w\in L(P')$ folgt also $w\in L(P)$ oder $w\in\cont (\StET
  (P))\subseteq \ET (P)$.
\end{Prop}
\begin{proof}
  Die Existenz des Traces in $P$ soll induktiv über die einzelnen Transitionen
  bewiesen werden. Die entsprechenden Behauptung für die Induktion lautet: Für
  alle $j\in\{0,1,\dots n-1\}$ mit $p'_j\mathcal{R} p_j$ gibt es eine Transition
  $p_j \may{\alpha _{j+1}}_P p_{j+1}$ mit $p'_{j+1}\mathcal{R} p_{j+1}$ oder
  $p_j$ ist in $E_P$ enthalten.\\
  Die Verankerung $p'_0\mathcal{R} p_0$ folgt direkt aus der Voraussetzung, das
  $\mathcal{R}$ eine as"=Verfeinerungs"=Relation für $P'$ und $P$ ist.\\
  Es kann also nun davon ausgegangen werden, dass $p'_j\mathcal{R} p_j$ für ein
  $j$ mit $0\leq j < n$ erfüllt ist.
  \begin{itemize}
    \item Fall 1 ($p_j\in E_P$): Die Behauptung der Induktion ist bereits
      erfüllt.
    \item Fall 2 ($p_j\notin E_P$): In der Verfeinerung $P'$ existiert die
      Transition $p'_j \may[\alpha _{j+1}]_{P'} p'_{j+1}$. Mit
      Definition~\ref{SimDef}.3 folgt daraus die Existenz der Transition $p_j
      \may[\alpha _{j+1}]_P p_{j+1}$ in der Spezifikation $P$ mit $p'_{j+1}
      \mathcal{R} p_{j+1}$.
  \end{itemize}
  Für jedes $w$ aus $L(P')$ gibt es einen Ablauf wie er in dieser Proposition
  vorausgesetzt wurde, so dass $w=(\alpha _1\alpha _2\dots \alpha
  _n)|_{\Sigma}$ gilt. Im Fall 1 wurden bis $p_j$ die Aktionen $\alpha$ in
  ihrer Reihenfolge ausgeführt. Es gibt also ein Präfix $v$ von $w$, dass in
  $P$ zu einem Fehler"=Zustand führt und somit $v\in\StET (P)$ erfüllt. $w$ ist
  eine Fortsetzung von $v$ somit gilt die Aussage $w\in\cont (\StET
  (P))\subseteq \ET (P)$ ebenfalls. Falls der Fall 2 für $j = n-1$ angewendet
  werden kann, folgt daraus $p'_n\mathcal{R} p_n$ und $\alpha _1\alpha _2\dots
  \alpha _n$ sind die Transitionsbeschriftungen, die vom Startzustand $p_0$ zum
  Zustand $p_n$ in $P$ führen. Das Wort $w$ ist also in $P$ ausführbar und
  somit in der Sprache $L(P)$ enthalten.
\end{proof}
...
\begin{proof}\mbox{}\\
  ...
  \begin{itemize}
    \item Fall 1 ($w\in\PrET (P')$): In $P'$ existiert eine Verlängerung $v\in
      ...
      ausführbar ($wv\in L(P')$). Es kann somit die
      Proposition~\ref{AblaefeVerfSpezProp} angewendet werden, wobei
      ...
      durch den Beweis von~\ref{AblaefeVerfSpezProp}. Da $p'_n\in E_{P'}$ gilt,
    ...
    \item Fall 2 ($w\in\MIT (P')\backslash\PrET (P')$): Da $w$ in $\MIT (P')$
      ...
      die Proposition~\ref{AblaefeVerfSpezProp} angewendet werden, wobei das
      ...
      die Implementierung der $a$ Transition fordern, dies würde einen
      Widerspruch zu $va\in\MIT (P')$ darstellen. Es gilt also $p\nmust[a]$ und
    ...
    \item Fall I ($v_1\in\MIT _1$): Da $v_1\in\MIT _1$ gilt, muss $v_1$
      ...
      \begin{itemize}
        ...
        \item Fall Id) ($a\in O_2$): Es gilt $v'_2=v''_2a$ und $p_{02}
          ...
          $(p_{01},p_{02}) \weakmay[v'']_{12} (p_1,p_2)$ mit $p_2\may[a]_1$,
