...
Dieses Kapitel erweitert die Präkongruenz für Error bei \EIO{}s aus
z.B.~\cite{Schinko2016BA} auf die hier betrachten \MEIO{}s.
...
\begin{proof}\mbox{}\\
  ...
  \begin{itemize}
    \item Fall 1 ($w\in\PrET (P')$): In $P'$ existiert eine Verlängerung $v\in
      ...
      alle Zustände außer $p'_n$ gilt, dass sie keine Fehler-Zustände sind. Es
      wird nun versucht via Induktion zu zeigen, dass man den strikten
      Fehler-Trace in $P$ nachmachen kann. Es soll dafür beweisen werden, falls
      $p_j\notin E_P$ gilt und $p'_j\mathcal{R} p_j$ bereits nachgewiesen
      wurde, dass auch $p'_{j+1}\mathcal{R} p_{j+1}$ gelten muss für ein
      $p_{j+1}\in P$ und $0\leq j < n$. Für $j=0$ gilt bereits $p'_j\mathcal{R}
      p_j$. Falls ein $p_j$ vor $p_n$ in $E_P$ enthalten ist, dann ist ein
      Präfix von $wv$ ein strikter Fehler-Trace in $P$ und mit $w=\prune(wv)$
      ist somit ein Präfix von $w$ in $\ET (P)$ enthalten. Da \ET{} unter
      ...
      $p'_{j+1}\mathcal{R} p_{j+1}$ gilt. Aus der Induktion kann nun gefolgert
      werden, dass $p'_n\mathcal{R} p_n$ gelten muss. Da $p'_n\in E_{P'}$ gilt,
    ...
    \item Fall 2 ($w\in\MIT (P')\backslash\PrET (P')$): Da $w$ in $\MIT (P')$
      ...
      Induktion wie im letzten Fall für $0\leq j < n-1$ kann $p'_{n-1}
      \mathcal{R} p_{n-1}$ für einen Zustand $p_{n-1}$ in $P$ gefolgert werden,
      falls nicht davor ein $p_j$ in der Induktion auftaucht, für das $p_j\in
      E_P$ gilt. Falls eines der $p_j$ mit $0\leq j \leq n-1$ ein
      Fehler-Zustand ist, gilt $w\in\ET (P)$ mit der analogen Argumentation wie
      im letzten Fall. Es wird im folgenden davon ausgegangen, dass $p_{n-1}$
      mit $p'_{n-1}$ in der Relation $\mathcal{R}$ steht und keine
      Fehler-Zustände aufgetreten sind. Falls $a$ für $p_{n-1}$ eine ausgehende
      must"=Transition wäre, würde~\ref{SimDef}~2.\ auch für $p'_{n-1}$ die
      Implementierung der $a$ Transition fordern und somit würde nicht
      $va\in\MIT (P')$ gelten. Es gilt also $p\nmust[a]$ und $va\in\MIT (P)$.
