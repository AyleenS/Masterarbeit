...
\begin{proof}\mbox{}
  \begin{enumerate}
    ...
    \item Da der erste Punkt dieser Proposition bereits beweisen wurde, gilt,
      ...
      wird, gilt die Inklusion der daraus entstanden Mengen weiterhin. Dies
      entspricht der Behauptung dies Punktes.
...
\begin{proof}
  ...
  Für $n=0$ gibt es keine $\alpha$s, die zuvor ausgeführt werden müssen, und
  $p_0\mathcal{R} q_0$ folgt direkt aus der Voraussetzung, das $\mathcal{R}$
  eine schwache as"=Verfeinerungs"=Relation zwischen $P$ und $Q$ ist.\\
  Die Aussage des Lemmas über die Abläufe gilt für ein $j$ mit $0\leq j<n$.
  \begin{itemize}
    ...
    \item Fall 3 ($p_j \mathcal{R} q_j \land q_j\notin E_Q \land \neg\exists 0
      \leq k < j: q_k\in E_Q$): In der Verfeinerung $P$ existiert die
      ...
      $(p_{j+1},q_{j+1})\in\mathcal{R}$. Die schwach"=nachlaufenden
      may"=Transition ist nur eine spezielle Form der schwachen may"=Transition
      es gilt also auch $q_j \weakmay[\widehat{\alpha _{j+1}}]_Q q_{j+1}$.
...
\begin{proof}
  ...
  \may[\alpha _{j+1}]_Q q_{j+1}$. Es folgt also der Ablauf, der hier gezeigt
  werden sollte, falls kein Fehler"=Zustand in $Q$ angetroffen wird.
\end{proof}
...
\begin{Kor}[unendliche Abläufe in as-Verfeinerungen und Spezifikationen]
  ...
  \may[\alpha _2]_Q \dots$ in $Q$ zu gematched werden oder es gibt ein endliche
  Anfangsstück des Ablaufes, das in $Q$ einen Fehler"=Zustand führt.
\end{Kor}

Aus den Propositionen für die Sprache und die in der Menge \ET{} relevanten
Traces konnte für die Vereinigung der gleichen Mengen über die
Implementierungen immer nur eine Inklusionsrichtung gefolgert werden, da die
Definition~\ref{SimDef} nach einen Fehler-Zustand in $P$ beliebiges Verhalten
in dessen as"=Implementierungen zulässt. Mit dem Einsatz der \cont{}-Funktion
zum beliebigen fortsetzten der Traces kann dies ausgeglichen werden. Somit
gilt, wie die nächsten Proposition zeigt, für die Fehler-Traces und die
Fehler-geflutete Sprache Gleichheit und nicht nur die Inklusion.
...
\begin{proof}\mbox{}\\
  ...
  Transitionssystemen. Es muss $p'_0\mathcal{R}p_0$ gelten für die
  ...
  \begin{itemize}
    \item Fall 1 ($w\in\PrET (P')$): In $P'$ existiert eine Verlängerung $v\in
      ...
      draus mit~\ref{SimDef}.1, dass bereits $p_n$ in $E_P$ enthalten sein
      muss. Es gilt also $wv\in\StET (P)$ und mit der Argumentation von oben
      folgt daraus $w\in\ET (P)$.
    \item Fall 2 ($w\in\MIT (P')\backslash\PrET (P')$): Da $w$ in $\MIT (P')$
      ...
      wäre, würde~\ref{SimDef}.2 auch für $p'_n$ die Implementierung der $a$
      Transition fordern, dies wäre jedoch ein Widerspruch zur Wahl des
...
\begin{proof}\mbox{}\\
  1. \glqq$\subseteq$\grqq{}:\\
  Da beide Seiten der Gleichung unter der Fortsetzung \cont{}-Funktion
  abgeschlossen sind, genügt es ein präfix-minimales Element $w$ aus $\ET
  _{12}$ zu betrachten. Diese Element ist aufgrund der Definition der Menge der
  ...
  \item Fall 2 ($w\in\PrET _{12}$): Aus der Definitionen von \PrET{} und
      ...
      geerbten oder einen neuen Fehler handelt. Bei einem geerbten wäre bereits
      einer der beiden Zustände $p'_1$ bzw.\ $p'_2$ ein Fehler"=Zustand
  ...
  \begin{itemize}
    ...
    \item Fall II ($v_1\in\PrET _1$): $\exists u_1\in O_1^*: p_{01}
      ...
      Fall Ic) und wird hier nicht weiter betrachtet. Es für alle anderen Fälle
