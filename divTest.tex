...
\begin{proof}[Beweis zu Satz~\ref{DivSemSatz}]\mbox{}\\
  ...
  Die hier zu zeigende Inklusion lässt sich auf den Beweis von $\StQT (\DF
  (P_1)\|\DF (P_2)) \backslash \ET (\DF (P_1)\|\DF (P_2)) \subseteq\QET (\DF
  (P_{12}))$ reduzieren. Ein beliebiges $w$ aus $\StQT (\DF (P_1)\|\DF (P_2))
  \backslash \ET (\DF (P_1)\|\DF (P_2))$ ist die Beschriftung eines Ablaufes
  von $\DF (P_1)\|\DF (P_2)$, der zu einem Zustand $(p_1,p_2)\in Qui _{\DF
  (P_1)\|\DF (P_2)}\backslash E_{\DF (P_1)\|\DF (P_2)}$ führt. Die Zustände
  $p_1$ und $p_2$ in $\DF (P_1)$ bzw.\ $\DF (P_2)$ sind still durch
