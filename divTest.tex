...
Die Relation \DRel{} ist keine Einschränkung von \ERel{} so wie \QRel{}. Diese
...
\begin{proof}[Beweis zu Satz~\ref{DivSemSatz}]\mbox{}\\
  ...
  _P$ und $\EDL _P = \EL _P$. Somit lassen sich die hier zu beweisenden
  Aussagen auf die des Satzes~\ref{StilleSemSatz} reduzieren.
  \begin{enumerate}
    \item $\begin{aligned}[t]
        \EDT _{12}&\overset{\ref{DivNormProp}}{=} \EDT _{\DF (P_{12})}\\
        &\overset{\ref{QDRelDivNormLem}}{=} \ET _{\DF (P_{12})}\\
        &\overset{\ref{ParallDivNormKommLem}}{=} \ET _{\DF (P_1)\|\DF (P_2)}\\
        &\hspace{-0.05cm}\overset{\ref{StilleSemSatz}.1}{=} \cont (\prune ((\ET
        _{\DF (P_1)}\|\EL _{\DF (P_2)})\cup (\EL _{\DF (P_1)}\|\ET _{\DF
        (P_2)})))\\
        &\overset{\ref{QDRelDivNormLem}}{=} \cont (\prune ((\EDT _{\DF
        (P_1)}\|\EDL _{\DF (P_2)})\cup (\EDL _{\DF (P_1)}\|\EDT _{\DF
        (P_2)})))\\
        &\overset{\ref{DivNormProp}}{=} \cont (\prune ((\EDT _1\|\EDL _2)\cup
        (\EDL _1\|\EDT _2))),
    \end{aligned}$
    \item $\begin{aligned}[t]
        \QDT _{12}&\overset{\ref{DivNormProp}}{=} \QDT _{\DF (P_{12})}\\
        &\overset{\ref{QDRelDivNormLem}}{=} \QET _{\DF (P_{12})}\\
        &\overset{\ref{ParallDivNormKommLem}}{=} \QET _{\DF (P_1)\|\DF (P_2)}\\
        &\hspace{-0.05cm}\overset{\ref{StilleSemSatz}.2}{=} (\QET _{\DF
        (P_1)}\|\QET _{\DF (P_2)})\cup \ET _{\DF (P_1)\|\DF (P_2)}\\
        &\overset{\ref{ParallDivNormKommLem}}{=} (\QET _{\DF (P_1)}\|\QET _{\DF
        (P_2)})\cup \EDT _{\DF (P_{12})}\\
        &\overset{\ref{QDRelDivNormLem}}{=} (\QDT _{\DF (P_1)}\|\QDT _{\DF
        (P_2)})\cup \EDT _{\DF (P_{12})}\\
        &\overset{\ref{DivNormProp}}{=} (\QDT _1\|\QDT _2)\cup \EDT _{12},
    \end{aligned}$
    \item $\begin{aligned}[t]
        \EDL _{12}&\overset{\ref{DivNormProp}}{=} \EDL _{\DF (P_{12})}\\
        &\overset{\ref{QDRelDivNormLem}}{=} \EL _{\DF (P_{12})}\\
        &\overset{\ref{ParallDivNormKommLem}}{=} \EL _{\DF (P_1)\|\DF (P_2)}\\
        &\hspace{-0.05cm}\overset{\ref{StilleSemSatz}.3}{=} (\EL _{\DF
        (P_1)}\|\EL _{\DF (P_2)})\cup \ET _{\DF (P_1)\|\DF (P_2)}\\
        &\overset{\ref{ParallDivNormKommLem}}{=} (\EL _{\DF (P_1)}\|\EL _{\DF
        (P_2)})\cup \EDT _{\DF (P_{12})}\\
        &\overset{\ref{QDRelDivNormLem}}{=} (\EL _1\|\EL _2)\cup \EDT _{\DF
        (P_{12})}\\
        &\overset{\ref{DivNormProp}}{=} (\EL _1\|\EL _2)\cup \EDT _{12}.
    \end{aligned}$
  \end{enumerate}
\end{proof}
    ...
    \item Fall 2 ($w=x_1\dots x_n x_{n+1}\in\Sigma ^+$ mit $n\geq 0$ und
      ...
      \begin{itemize}
        ...
        \item $\begin{aligned}[t]
            ...
            &\cup\left\{(p_j,\chi ,p_{n+1})\mid 0\leq j\leq n+1\right\},
        \end{aligned}$
        \item $E_T=\emptyset$.
      \end{itemize}
      \begin{figure} [h!tbp]
      ...
      \begin{itemize}
        \item Fall 2c) \big($w\in\PrDT _{P'_1}$\big): In $P'_1\|T$ erhält man
          $(p_{01},p_0) \weakmay[w] (p'',p_{n+1})\weakmay[v] (p',p_{n+1})$ für
          $v\in O^*$ und $p'\in Div_1$. Die Einschränkung, dass $v$ keine
          ...
          $wv\in\StDT (P'_1\|T)$. Da alle Aktionen aus $w$ synchronisiert
          werden und $I_T\cap I=\emptyset$ gilt $x_1,\dots , x_n, x_{n+1}\in
          O_{P'_1\|T}$. Da zusätzlich $v$ in $O^*$ enthalten ist, folgt $v\in
      ...
      \begin{itemize}
        ...
        \item Fall 2ii) ($\varepsilon\in\ET (P'_2\|T)$ wegen geerbtem Fehler):
          ...
          _{P'_2}$ enthalten und mit Proposition~\ref{DivSemProp}.2 ist ein
          Präfix von $w$ in $\EDT _2$ enthalten.
        \item Fall 2iii) ($\varepsilon\in\DT (P'_2\|T)$): Da $T$ keine
          $\tau$-Transitionen besitzt, kann das Divergenz"=Verhalten nur von
          $P'_2$ geerbt sein. $T$ hat $x_1\dots x_j v$ ausgeführt mit $v\in (O
          \cup \{\chi\})^*$ und ebenso hat $P'_2$ den Weg $x_1\dots
          x_j v|_{\Sigma _2}$ ausgeführt. Durch dies hat $P'_2$ einen Zustand
          aus $Div_{P'_2}$ erreicht. Es gilt dann $\prune (x_1\dots
          x_j v|_{\Sigma _2}) = \prune (x_1\dots x_j)\in\PrDT _{P'_2} \subseteq
          \DT _{P'_2}$, da $v|_{\Sigma _2}$ in $O^*$ enthalten ist. Da
          ...
          _{P'_2}$ gilt, ist in diesem Fall das Präfix $x_1\dots x_j$ von $w$
          in $\EDT _{P'_2}$ enthalten. Mit Proposition~\ref{DivSemProp}.2 folgt
          daraus, dass ein Präfix von $w$ in $\EDT _2$ enthalten ist.
      \end{itemize}
  \end{itemize}
  ...
  \begin{center}
    \begin{tikzpicture}[scale = 3]
      \matrix (m) [matrix of math nodes,row sep=2cm,column sep=4cm]{%
        P_1\DRel P_2 & P_1 \text{ Divergenz-verfeinert } P_2 \\
