...
\begin{Def}[Test und Verfeinerung für Divergenz]
  ...
  \emph{stillstand-} und \emph{divergenz-frei}, wenn kein Fehler-, Stille- oder
  Divergenz-Zustand lokal erreichbar ist.\\
...
die strikten Stille"=Traces mit diesen beiden Trace-Mengen geflutet.
...
\begin{Kor}[lokale Divergenz Erreichbarkeit]\mbox{}
  ...
  \begin{enumerate}[(i)]
    ...
    \item Falls in einem \MEIO{} $P$ Divergenz lokal erreichbar ist, dann
      ...
      erreichbar ist.
    \item Falls ein \MEIO{} $P$ einen lokal erreichbaren Stille-Zustand besitzt,
      dann existiert auch eine as"=Implementierung, in der ein Stille-Zustand
      lokal erreichbar ist.
    \item Falls es eine as"=Implementierung von $P$ gibt, die einen Fehler,
      Stille oder Divergenz lokal erreicht, dann ist auch ein Fehler, Stille oder
...
\begin{proof}\mbox{}
  \begin{enumerate}[(i)]
    ...
    \item In $P'\in\asimp (P)$ sei ein Fehler-, Stille- oder Divergenz-Zustand
      ...
      in $P$ ein Fehler-, Stille- oder Divergenz-Zustand lokal erreichbar sein,
...
\begin{proof}
  ...
  Fehlerhaftigkeit\grqq{} hier ein Fehler, Stille oder Divergenz sein. Analog
  verhält es sich, wenn in der Parallelkomposition einer as"=Implementierung
  von $P_1$ mit einem Test $T$ ein Divergenz-Zustand oder Stille-Zustand lokal
  ...
  \begin{itemize}
    \item Fall 1 ($w=\varepsilon$): Es handelt sich um einen lokal erreichbaren
      ...
      $T$ ist in einer Parallelkomposition mit $T$ kein Stille-Zustand möglich.
    ...
    \item Fall 2 ($w=x_1\dots x_n x_{n+1}\in\Sigma ^+$ mit $n\geq 0$ und
      ...
      \end{center}
      \end{figure}
      Die Mengen der Divergenz- und Stille-Zustände des hier betrachteten $T$s
      ...
      von $T$ kann es in Komposition mit $T$ keine Stille-Zustände geben. Die
  ...
  Proposition~\ref{StilleTraceProp} und~\ref{DivSemProp}~2.\ auch für eine
  ...
  $P'_1\|T$, wenn in $T$ ebenfalls ein Stille"=Zustand durch $w$ erreicht wird.\\
  ...
  gilt werden des ersten Punkte von Lemma~\ref{StilleZustLem}, dass auch in der
  Parallelkomposition $P'_1\|T$ ein Stille-Zustand mit $w$ erreicht wird. Da es
  sich bei allen in $w$ befindlichen Aktionen um synchronisieren Aktionen
  ...
  Stille-Zustand lokal erreicht werden. Da $w\notin\EDT _{P'_1}$, kann auf dem
  Weg, der mit $w$ im Transitionssystem $P'_1$ zurück gelegt wird, kein Fehler-
  oder Divergenz-Zustand lokal erreicht werden. Es kann also weder von $P'_1$
  noch von $T$ ein Fehler oder Divergenz auf diesem Weg geerbt werden oder neu
  entstehen. Da ein Stille"=Zustand in $P'_1\|T$ lokal erreichbar ist, erfüllt
  ...
  Fehler, Stille oder Divergenz handelt.
  \begin{itemize}
    \item Fall a) ($\varepsilon \in \ET (P'_2\|T)$): Der lokal erreichbare
      ...
      ausführbar sein. Der Fehler kann sowohl von $P'_2$ geerbt sein, wie durch
      fehlende Synchronisations-Sicherstellung als neuer Fehler in der
    ...
    \item Fall c) (Stille-Zustand lokal erreichbar in $P'_2\|T$ und $\varepsilon
      ...
      werden, dass auch in $P'_2$ ein Stille"=Zustand mit $w$ erreichbar sein
  ...
  \begin{itemize}
    ...
    \item Fall 2 ($w = x_1\dots x_n$ mit $n \geq 1$): Die Konstruktion des
      ...
      Zustand\grqq{} hat. Wie oben bereits erwähnt, kommt Stille als \glqq
...
\begin{proof}
  ...
  ein Fehler-, Stille- oder Divergenz-Zustand lokal erreichbar ist. $P_1\DRel
  P_2$ impliziert, dass $w\in\QDT _2$ gilt, wenn $w\in\QDT _1$ gilt. Somit ist
  ein Fehler-, Stille- oder Divergenz-Zustand nur dann in $P_1$ lokal erreichbar,
  ...
  Fehler-, Stille- oder Divergenz"=Zustand. Der \glqq fehlerhafte Zustand\grqq
  einer as"=Implementierung $P'_1$ tritt in der Parallelkomposition mit $T$ auf
  jeden Fall auf, wenn es sich um einen Fehler oder Divergenz handelt. Bei Stille
  zeigt sich das \glqq Fehlverhalten\grqq{} nur bei Tests $T$, die einen
  analogen Stille"=Zustand enthalten. Mit der Relation \DRel{} gibt es auch in
  $P_2$ und somit auch in einer as"=Implementierung $P'_2$ davon einen lokal
  ...
  Falls es sich in $P_2$ und somit auch in $P'_2$ nur um Stille handelt und kein
  Fehler oder Divergenz lokal erreichbar sind, dann muss auch bereits in $P_1$
  bzw.\ $P'_1$ Stille das einzige \glqq Fehlverhalten\grqq{} gewesen sein, dass
  ...
  Satz~\ref{StilleTestVerfSatz} anwendbar. Es gilt also $\neg P_1 \DsatAs T
...
gezeigt, die sich zu einem Fing schließt. Jedoch ändert sich an der Begründung
...
\begin{proof}\mbox{}\\
  ...
  ersten Punkt zu zeigen, dass ein beliebiges präfix-minimales $w$ aus $\EDT _P$
  auch in $\EDT _Q$ enthalten ist. $w$ kann ein Element aus $\ET _P$ oder $\DT
  _P$ sein. Für $w$ aus $\ET _P$ folgt mit Satz~\ref{ZusammenhFehlerSatz}
  bereits $w\in\ET _Q$. Es kann also im folgenden davon ausgegangen werden,
  ...
  \ET _Q \subseteq \EDT _Q$. Es kann für den restlichen Beweis davon ausgegangen
  ...
  Folge an $\tau$s ausführbar. Die Definition~\ref{SimDef}~3.\ erzwingt, dass
  ...
  ausführbar ist, da die Transitionen von $P$ Transitionen in $Q$ entsprechen
  ...
  $\asRel \Rightarrow \wasRel$ und wegen der Inklusion $\EL _Q \subseteq \EDL
  ...
  dem Alphabet $\Sigma$ möglich sind. Es gibt keine Stille"=Trace. Somit gilt
  also $P \DRel Q$.\\
  ...
  gematched werden. Es also möglich ist, eine unendliche $\tau$ Folge ohne eine
