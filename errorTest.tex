...
\end{Satz}
\begin{proof}
  Um zu zeigen, dass $P_1$ $P_2$ Fehler-verfeinert, muss nachgewiesen werden,
  dass für alle Test $T$ $P_2\EsatAs T \Rightarrow P_1\EsatAs T$ gilt. Diese
  Aussage ist analog zu für alle Test $T$ gilt $\neg P_1\EsatAs T \Rightarrow
  \neg P_2\EsatAs T$. Es wird ein beliebiger Test $T$ gewählt für den $\neg P_1
  \EsatAs T$ gilt. Es muss also eine as"=Implementierung $P'_1$ von $P_1$
  geben, die in der Parallelkomposition einen Fehler lokal erreicht. Es gilt
  also $\varepsilon\in\ET _{P'_1\|T}$. Nach Satz~\ref{KommFehlerSemSatz}.1 gilt
  $\ET _{P'_1\|T} = \cont (\prune ((\ET _{P'_1}\|\EL _T)\cup (\EL _{P'_1}\|\ET
  _T)))$. $\varepsilon$ ist auf jeden Fall präfix-minimal, das \cont{} hat also
  keinen Einfluss. Mit Proposition~\ref{KommSemProp} folgt $\varepsilon \in
  \prune ((\ET _1\|\EL _T)\cup (\EL _1\|\ET _T))$. Es gibt also ein $w\in
  O_{P_1\|T}^*$, dass entweder in der Menge $\ET _1\|\EL _T$ oder in der Menge
  $\EL _1\|\ET _T$ enthalten ist. Es gibt also Wörter $w_1$ und $w_T$ mit $w\in
  ...
  _2$. Es gibt durch Proposition~\ref{KommSemProp} eine as"=Implementierung
  $P'_2$ von $P_2$ für die $w_1\in\ET _{P'_2}$ bzw.\ $w_1\in\ET _{P'_2}$ gilt.
  In Parallelkomposition mit $T$ ist das Wort $w$ also in der Menge $(\ET
  _{P'_2}\|\EL _T)\cup (\EL _{P'_2}\|\ET _T)$ enthalten. $P_1$ und $P_2$ müssen
  die gleiche Signatur besitzen, es gilt also auch $w\in O_{P'_2\|T}$ und somit
  $\varepsilon\in\prune ((\ET _{P'_2}\|\EL _T)\cup (\EL _{P'_2}\|\ET _T)))$. Es
  ist also in $P'_2\|T$ ein Fehler lokal erreichbar und es folgt $\neg P'_2
  \EsatAs T$.
\end{proof}
...
Die in Abbildung~\ref{KommTestFolgerungskette} dargestellten Zusammenhänge
