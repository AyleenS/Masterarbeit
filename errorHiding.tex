    ...
    \item Die \ET{}-Mengen sind unter \cont{} abgeschlossen. Es genügt also ein
      ...
      \begin{itemize}
        \item Fall 1 ($w'\in\PrET _P$): Da $w'$ in $P$ ausführbar ist, gibt es
          ...
          $w=w'|_{\Sigma\backslash X} = \prune ((w'v)|_{\Sigma\backslash X})$.
      ...
      \end{itemize}
      Ein präfix-minimales $w$ aus $\ET _{P/X}$ kann ohne den letzten Input
      einen Zustand, für den nicht alle Input"=Transitionen sichergestellt sind
      oder über eine Verlängerung einen Fehler"=Zustand in $P/X$ erreichen. Für
      das Präfix von $w$ ohne den letzten Input oder das gesamte $w$ und die
      mögliche Verlängerung existiert ein Ablauf der Form wie in (i). In dem
      $\tau$s auf diesem Weg durch Outputs aus $X$ ersetzt werden, kann der
